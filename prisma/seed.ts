...
import { PUBLIC_SERVICE_LINK_SEED, PUBLIC_SERVICE_PROCEDURE_SEED, toProcedureRecord } from "./public-service-dvcqg-data";
...
async function main() {
  ...
  // Dịch vụ công — nội dung lấy từ prisma/public-service-dvcqg-data.ts (Cổng DVCQG).
  // prisma:seed chỉ create nháp (isActive=false), không ghi đè bản admin đã sửa.
  // Để bật công khai và cập nhật hướng dẫn: pnpm seed:public-services
  console.log("Seeding thủ tục dịch vụ công (nguồn DVCQG, nháp — chạy pnpm seed:public-services để bật)...");
  for (const p of PUBLIC_SERVICE_PROCEDURE_SEED) {
    await prisma.publicServiceProcedure.upsert({
      where: { slug: p.slug },
      update: {},
      create: toProcedureRecord(p, false)
    });
  }

  console.log("Seeding kho liên kết dịch vụ công...");
